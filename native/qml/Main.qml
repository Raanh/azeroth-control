...
ApplicationWindow {
    ...
    property string coaRepackPath: "/home/deck/Downloads/CoA-Repack"
    ...
    Item {
        ...
        Rectangle {
            ...
            Column {
                ...
                Text { visible: root.installProviderId === "azerothcore-coa"; text: "CoA Repack folder (matching server data)"; color: root.muted; font.pixelSize: 17 * root.s }
                TextField { id: coaRepack; visible: root.installProviderId === "azerothcore-coa"; width: 800 * root.s; height: 58 * root.s; text: root.coaRepackPath; font.pixelSize: 17 * root.s; selectByMouse: true; onTextChanged: root.coaRepackPath = text }
                ...
                Text { visible: root.installProviderId === "azerothcore-coa"; width: 940 * root.s; text: "Copy the complete CoA-Repack folder from the forum to this Deck and use it here. Azeroth Control imports its matching Data and clean database snapshot, but builds and runs the server natively on Linux; Windows EXEs are never used."; color: root.gold; font.pixelSize: 15 * root.s; wrapMode: Text.Wrap }
                ...
                Row { spacing: 12 * root.s
                    AzButton { id: installButton; text: control.installRunning ? "Installing…" : "Start / Resume installation"; primary: true; width: 290 * root.s; height: 62 * root.s; font.pixelSize: 18 * root.s; enabled: !control.installRunning && !control.busy; onClicked: { var profiles = root.installProviderId === "azerothcore-coa" ? root.coaProfiles : root.installProfiles; var chosenProfile = profiles[installProfile.selectedIndex]; var chosenModules = root.installProviderId === "azerothcore-coa" ? ["ascension-compat"].concat(root.installAutoBalance ? ["autobalance"] : []) : ["playerbots", "dungeon-clear", "aoe-loot", "transmog", "learn-spells", "auction-house", "multibot-bridge"]; control.installServer({"provider": root.installProviderId, "profile": chosenProfile.id, "clientPath": installClient.text, "coaRepackPath": root.installProviderId === "azerothcore-coa" ? coaRepack.text : "", "bots": root.installProviderId === "azerothcore-coa" ? 0 : Math.round(installBots.value), "modules": chosenModules, "installRoot": "/home/deck/.local/share/azeroth-control", "serverName": root.installProviderId === "azerothcore-coa" ? "Conquest of Azeroth" : "Azeroth " + chosenProfile.name, "accountName": root.firstAccount, "accountPassword": root.firstPassword}) } KeyNavigation.up: installBots; KeyNavigation.right: pauseInstallButton }
