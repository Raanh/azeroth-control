...
type Selection = { mode: 'new' | 'import'; installRoot: string; clientPath: string; profile: string; bots: number; modules: string[]; stopWithGame: boolean; steamShortcuts: boolean; serverId?: string; serverName?: string; accountName?: string; accountPassword?: string; adminAccount?: boolean };
...
export default function Installer({ onComplete, onCancel }: { onComplete: () => void; onCancel: () => void }) {
  ...
  const [selection, setSelection] = useState<Selection>({ mode: 'new', installRoot: '', clientPath: '', profile: 'progression', bots: 500, modules: [], stopWithGame: true, steamShortcuts: true, serverId: `server-${crypto.randomUUID().slice(0, 8)}`, serverName: 'Azeroth Progression', accountName: 'player', accountPassword: 'player', adminAccount: false });
